                            ...
                            <div class="card-body">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" name="order_code"
                                        class="form-control font-weight-bold text-primary" value="{{ $order_code }}"
                                        readonly required>
                                </div>

                                <div class="form-group mb-3">
                                    <label class="font-weight-bold">Tipe Pelanggan <span
                                            class="text-danger">*</span></label>
                                    <div class="d-flex mb-2">
                                        <div class="custom-control custom-radio mr-3">
                                            <input type="radio" id="customer_type_old" name="customer_type"
                                                class="custom-control-input" value="old"
                                                {{ old('customer_type', 'old') === 'old' ? 'checked' : '' }}>
                                            <label class="custom-control-label" for="customer_type_old"
                                                style="cursor: pointer;">Pelanggan Terdaftar</label>
                                        </div>
                                        <div class="custom-control custom-radio">
                                            <input type="radio" id="customer_type_new" name="customer_type"
                                                class="custom-control-input" value="new"
                                                {{ old('customer_type') === 'new' ? 'checked' : '' }}>
                                            <label class="custom-control-label" for="customer_type_new"
                                                style="cursor: pointer;">Pelanggan Baru</label>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div id="old-customer-wrapper" class="form-group mb-3">
                                    ...
                                    <select name="id_customer" id="id_customer" class="form-control" required>
                                        ...
                                        @foreach ($customers as $c)
                                            <option value="{{ $c->id }}"
                                                {{ old('id_customer') == $c->id ? 'selected' : '' }}>{{ $c->customer_name }}
                                                - {{ $c->phone }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div id="new-customer-form"
                                    style="display: none; border-left: 3px solid #6777ef; padding-left: 12px; margin-top: 15px; margin-bottom: 15px; background: #fafbfe; padding-top: 10px; padding-bottom: 10px; border-radius: 4px;">
                                    ...
                                    <div class="form-group mb-2">
                                        ...
                                        <input type="text" name="new_customer_name" id="new_customer_name"
                                            class="form-control form-control-sm" placeholder="Nama pelanggan baru"
                                    ...
                                    <div class="form-group mb-0">
                                        ...
                                        <textarea name="new_customer_address" id="new_customer_address" class="form-control form-control-sm"
                                            style="height: 60px;" placeholder="Alamat lengkap">{{ old('new_customer_address') }}</textarea>
                                    </div>
                                </div>
                                        ...
                                        <tr class="item-row">
                                            ...
                                            <td>
                                                <input type="number" name="qty[]" class="form-control qty-input"
                                                    min="0.001" step="any" value="1" required>
                                            </td>
                                            <td>
                                                <input type="text" name="notes[]" class="form-control notes-input"
                                                    placeholder="ex: Kain Luntur">
                                            </td>
                                            <td>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group mb-2">
                                            ...
                                            <input type="text" id="subtotal-display"
                                                class="form-control text-center font-weight-bold text-dark" readonly
                                                value="Rp 0">
                                        </div>

                                        <div class="form-group mb-2">
                                            <label class="font-weight-bold">Pajak (5%)</label>
                                            <input type="text" id="tax-display"
                                                class="form-control text-center font-weight-bold text-danger" readonly
                                                value="Rp 0">
                                        </div>
                                        ...
                                        <div class="form-group mb-3">
                                            <label class="font-weight-bold">Opsi Pembayaran <span
                                                    class="text-danger">*</span></label>
                                            <select name="payment_option" id="payment_option" class="form-control"
                                                required>
                                                <option value="upfront" selected>Bayar di Muka (Lunas)</option>
                                        ...
                                        <div class="form-group mb-3" id="order_pay_wrapper">
                                            <label class="font-weight-bold">
                                                Jumlah Uang yang Diterima (Rp) <span class="text-danger">*</span>
                                            </label>
                                        ...
                                        <div class="form-group mb-3" id="change_wrapper">
                                            <label class="font-weight-bold">Uang Kembalian</label>

                                            <input type="text" id="change_display"
                                                class="form-control font-weight-bold text-success" readonly value="Rp 0"
...
<script>
    $(document).ready(function() {
        ...
        // Trigger change event on page load to restore state
        ...
        $('#order_pay').on('input', function() {
            calculateChange();
        });

        // Pindahkan pemanggilan awal ini ke dalam document.ready agar aman dieksekusi
        calculateTotal();
        calculateChange();
    });

    // Deklarasi fungsi diletakkan di luar agar rapi
    ...
    function calculateChange() {
        let total = parseFloat($('#total-hidden').val()) || 0;
        let bayar = parseFloat($('#order_pay').val()) || 0;
        ...
        if (bayar >= total) {
            ...
            $('#change_display').val('Rp 0');
            $('#payment_note').text(
                'Uang kurang Rp ' + kurang.toLocaleString('id-ID')
            );
        }
    }
</script>
